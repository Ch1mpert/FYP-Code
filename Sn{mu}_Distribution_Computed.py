...
# for i in tqdm(range(math.floor((-n*math.sqrt(dt)/1.5)), math.floor((1.3*n*sp.sqrt(dt))))): 
#     x.append(i)
#     y=g(n, i, math.floor(c), math.floor(c), p, q)
#     dist.append(y)
...
#x_values = np.linspace(-n*math.sqrt(dt)/1.5, 1.3*n*(dt**0.5), 1000)
